...
Mg+2 vs. Fuel heatmaps in a case where TX and TL are separated.
	In this experiment, we attempt to see if we can create a TL-only by adding tetracycline, a 
known inhibitor of translation initiation. This time, the tetracycline is solubilized in DMSO.

Protocol:
1. Create an Echo picklist to dispense the following into each well, for 3 replicates per condition:
	- Some amount of tetracycline
	- 5nM DNA
	- Water to 10uL
2. Prepare master mix: 18rxns * 8.49uL/rxn * 1.1 excess = 168uL 
		...
		Mg (100mM)				15.84
		3-PGA (700mM)				8.48
3. Obtain stock of 80mg/mL tetracycline.
...
one well space between wells with different samples.
		...
		-----------	-------------------------		-----------
		G06		Tetracycline (3200X diluted)		28
		G08		DNA, 228ng/uL				33
		G10		Tetracycline (16X diluted)		28
		G12			NFW				65
...
Results: 
	- It appears that the 500ug/mL is the best concentration for inhibition of translation.
	- However, we must rule out the possibility that DMSO is affecting TX-TL.
	- At 500ug/mL tetracycline, we have DMSO concentration at a concentration of 0.625% (v/v).
	- We must repeat this experiment in a way such that DMSO concentration is 0.625% in all
	  wells.
	- Or, just do DMSO titrations (i.e. no tetracycline) to see when it begins to make a difference.